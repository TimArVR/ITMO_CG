...
void Game::init()
{
	player1 = new RectObject(this, new Vertex{ -GATE_SIZE + GATE_OFFSET, 0.0f, 255, 255, 255, 1 }, 0.03f, 0.4f);
	leftGate = new RectObject(this, new Vertex{ -GATE_SIZE, 0.0f, 0, 0, 0, 1 }, 0.01f, 2.0f);
	
	player2 = new RectObject(this, new Vertex{ GATE_SIZE - GATE_OFFSET, 0.0f, 255, 255, 255, 1 }, 0.03f, 0.4f);
	rightGate = new RectObject(this, new Vertex{ GATE_SIZE, 0.0f, 0, 0, 0, 1 }, 0.01f, 2.0f);

	topBorder = new RectObject(this, new Vertex{ 0.0f, BOUND_SIZE / 2, 0, 0, 0, 1 }, 2.0f, BOUND_OFFSET * 2);
	bottomBorder = new RectObject(this, new Vertex{ 0.0f, -BOUND_SIZE / 2, 0, 0, 0, 1 }, 2.0f, BOUND_OFFSET * 2);
	
	ball = new BallObject(this, new Vertex{ 0.0f, 0.0f, 0, 255, 0, 1 }, 30.0f);
...
void Game::updateBallPosition()
{
	...
	{
		//Right gate collision - count
		++player1_count;
		std::cout << "SCORE: \tPlayerOne  " << player1_count << " || PlayerTwo  " << player2_count << std::endl;

		goal(false);
	}
	if (newPos.x - (ball->getRadiusX() / 2) < -GATE_SIZE)
	{
		//Left gate collision - count
		++player2_count;
		std::cout << "SCORE: \tPlayerOne  " << player1_count << " || PlayerTwo  " << player2_count << std::endl;
